<template>
  <div style="position: fixed; right: 1.5em; bottom: 1.5em; z-index: 1">
    ...
    <v-btn :disabled="settingsAreEqual" color="primary" variant="elevated" @click="postSonarrSettings">Save</v-btn>
  </div>
    ...
    <v-card>
      ...
      <v-row class="ma-4" no-gutters>
        <v-col v-for="currentItem in filteredItems" cols="auto">
          <v-col v-if="getPosterImage(currentItem)" cols="12">
            <async-image :alt="`${currentItem.title} poster`" :src='getPosterImage(currentItem)'
    ...
    <v-expand-transition>
      <v-col v-if="monitoredNotCollapsed">
        ...
        <v-autocomplete v-model="newSettings.monitoring_amount"
                        ...
                        item-value="id" label="How much is being monitored ahead of users watch position"/>
...
const monitoringOptions = [
  {name: "Season", id: 0},
  {name: "3 Episodes", id: 1},
  {name: "6 Episodes", id: 2},
  {name: "Series", id: 3},
]
