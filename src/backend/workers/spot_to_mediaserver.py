...
def get_playlist_tracks(playlist_id):
    ...
    playlist_tracks = [
        ...
        if not playlist_track["track"].get("episode", False)
    ]
...
def process_playlist_tracks(playlist_tracks, media_server_tracks_by_title):
    ...
    for playlist_track in playlist_tracks:
        ...
        track_artists_set = {
            artist["name"] for artist in playlist_track["track"]["artists"]
        }
        ...
        for media_server_track in potential_matches:
            ...
            if fuzzy_str_match(track_album, media_server_track["album"]) or any(
                fuzzy_str_match(artist, media_server_artist)
                for artist in track_artists_set
                for media_server_artist in media_server_track["artists"]
            ):
...
def process_playlist_audio(
    playlist_tracks, existing_server_playlists, media_server_tracks_by_title, playlist
):
    track_ids, _ = process_playlist_tracks(
        playlist_tracks, media_server_tracks_by_title
    )
    ...
    if existing_playlist:
        ...
        logging_manager.log(
            f"Creating new playlist {playlist_name}", level=logging.DEBUG
        )
...
def process_playlist_video(
    ...
    track_ids, missing_tracks = process_playlist_tracks(
        playlist_tracks, media_server_tracks_by_title
    )
    ...
    if existing_playlist:
        logging_manager.log(
            f"Playlist {playlist_name} already exists", level=logging.DEBUG
        )
        playlist_id = existing_playlist["id"]
        existing_entries = media_server.get_items_from_playlist(playlist_id)
        existing_entries_ids = [entry["PlaylistItemId"] for entry in existing_entries]
    ...
    for missing_track in missing_tracks:
        download_music_video(
            missing_track["name"],
            [artist["name"] for artist in missing_track["artists"]],
        )
    ...
    for playlist in config.SPOTIFY.playlists:
        ...
        if playlist["type"] in ["audio", "both"]:
            if not media_server_audio_tracks_by_title:
                media_server_audio_tracks_by_title = index_tracks_by_title(
                    media_server.get_music_items()
                )
            process_playlist_audio(
                playlist_tracks,
                existing_server_playlists,
                media_server_audio_tracks_by_title,
                playlist,
            )
        if playlist["type"] in ["video", "both"]:
            if not media_server_video_tracks_by_title:
                media_server_video_tracks_by_title = index_tracks_by_title(
                    media_server.get_music_video_items()
                )
            process_playlist_video(
                playlist_tracks,
                existing_server_playlists,
                media_server_video_tracks_by_title,
                playlist,
            )
