...
def get_start_time(
    item: dict, decay_start_timer: int, played_items=None
) -> datetime.datetime:
    ...
    def get_shortest_time():
        ...
        added = datetime.datetime.strptime(
            item.get("added", "1000-01-01T00:00:00Z"), "%Y-%m-%dT%H:%M:%SZ"
        ).replace(tzinfo=datetime.timezone.utc)
        last_aired = datetime.datetime.strptime(
            item.get("lastAired", "1000-01-01T00:00:00Z"), "%Y-%m-%dT%H:%M:%SZ"
        ).replace(tzinfo=datetime.timezone.utc)
        in_cinemas = datetime.datetime.strptime(
            item.get("inCinemas", "1000-01-01T00:00:00Z"), "%Y-%m-%dT%H:%M:%SZ"
        ).replace(tzinfo=datetime.timezone.utc)
        digital_release = datetime.datetime.strptime(
            item.get("digitalRelease", "1000-01-01T00:00:00Z"), "%Y-%m-%dT%H:%M:%SZ"
        ).replace(tzinfo=datetime.timezone.utc)
    ...
    try:
        if decay_start_timer == 0:
            return datetime.datetime.strptime(
                item.get("added", "0000-01-01T00:00:00Z"), "%Y-%m-%dT%H:%M:%SZ"
            ).replace(tzinfo=datetime.timezone.utc)
...
def reassign_based_on_age(
    items, decay_days, decay_start_timer, played_items, source_list, target_list=None
) -> None:
...
def classify_items_by_decay(
    items: List[dict],
    decay_days: int,
    decay_start_timer: int,
    played_items: list,
    quality_profile: int,
    ...
    for item in items:
        start_time = get_start_time(
            item=item, decay_start_timer=decay_start_timer, played_items=played_items
        )
    ...
    result = [
        (profile, decay_profiles[profile])
        for profile in sorted(decay_profiles.keys(), reverse=True)
    ]
    return result
...
def link_arr_to_media_server(media_server_item: dict, arr_items: list) -> dict:
    ...
    year = (
        ...
        if year_match and 1900 <= int(year_match.group(1)) <= 2200
        else None
    )
...
def sort_tuples(
    tuples: List[Tuple[int, List[int]]], reverse: bool = True
) -> List[Tuple[int, List[int]]]:
    return sorted(tuples, key=lambda x: x[0], reverse=reverse)


def subtract_dicts(
    tuples: List[Tuple[int, List[Dict]]],
) -> List[Tuple[int, List[Dict]]]:
    previous_elements = []
    ...
    for i in range(len(tuples)):
        ...
        new_elements = [
            item for item in current_elements if item not in previous_elements
        ]
