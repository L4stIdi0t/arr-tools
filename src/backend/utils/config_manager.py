...
class ConfigManager:
    ...
    def write_default_configs(self):
        ...
        with open(
            str(self.config_file_path).replace(".json", "-default.json"), "w"
        ) as f:
    ...
    def upgrade_config(self, existing_config_data):
        ...
        current_version = existing_config_data.get("MISC", {}).get(
    ...
    def load_config_file(self):
        try:
            if not self.config_file_path.exists():
                ...
                with open(self.config_file_path) as json_file:
                    ...
                    if config_data["MISC"]["config_version"] != self.latest_version:
                        logging_manager.log(
                            "Config version mismatch, upgrading config",
                            level=logging.INFO,
                        )
            ...
            logging_manager.log(
                "Config file is invalid, writing default config", level=logging.WARNING
            )
            self.write_default_configs()
        except FileNotFoundError:
            logging_manager.log(
                "Config file not found, writing default config", level=logging.WARNING
            )
            self.write_default_configs()
    ...
    def save_config_file(self, config_data: settings.Config):
        ...
        logging_manager.log(
            json.dumps(config_data.dict(), indent=4),
            level=logging.DEBUG,
            print_message=False,
        )
