...
def get_info():
    ...
    quality_profiles = [
        ...
        for d in radarr.get_quality_profile()
    ]
    tags = [
        {k: v for k, v in d.items() if k in ["label", "id"]} for d in radarr.get_tag()
    ]
...
@router.get(
    "/settings",
    response_model=settings.RadarrSettings,
    description="Get Radarr settings",
)
