...
def get_info():
    ...
    quality_profiles = [
        {k: v for k, v in d.items() if k in ["name", "id"]}
        for d in sonarr.get_quality_profile()
    ]
    tags = [
        {k: v for k, v in d.items() if k in ["label", "id"]} for d in sonarr.get_tag()
    ]

    return {"quality_profiles": quality_profiles, "tags": tags}
...
def delete_item(
    ...
    importListExclusion: bool = Query(
        True, description="Whether to add the item to import list exclusion"
    ),
    deleteFiles: bool = Query(True, description="Whether to delete associated files"),
):
    sonarr.del_series(
        id, delete_files=deleteFiles, add_import_list_exclusion=importListExclusion
    )
...
@router.get(
    "/settings",
    response_model=settings.SonarrSettings,
    description="Get Sonarr settings",
)
...
@router.get(
    "/delete-unmonitored", description="Get the results of a dry run of deleting"
)
...
@router.delete(
    "/delete-unmonitored",
    description="Delete the unmonitored files WARNING, no cancel...",
)
